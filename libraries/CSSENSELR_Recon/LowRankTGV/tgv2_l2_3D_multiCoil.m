function [u e] = tgv2_l2_3D_multiCoil(datackkk,sense, kmask, alpha0, alpha1, maxits, reduction,Threshold)
% Primal dual TGV2 algorithm, as described in the TGV paper

% alpha0, alpha1, maxits, reduction TGV convergeance parameters
% 
% (c) 31.8.2010
...
[CN M N S] = size(datackkk); % numSamplesOnSpoke, numSamplesOnSpoke, nCh

v = zeros(size(datackkk));
p = zeros(M,N,S,3);
q = zeros(M,N,S,6);

u = zeros(M,N,S);

kmask_ckkk=reshape(kmask,[1 size(kmask)]);%permute(repmat(kmask,[1 1 1 CN]),[4,1,2,3]);
u_crrr=0*kmask_ckkk;
% for c=1:CN
%     %u=real(ifft(ifft(datakk.*kmask,[],1),[],2));
%     %u = u + conj(squeeze(sense(c,:,:,:))).*ifft(ifft(ifft(squeeze(datackkk(c,:,:,:)).*kmask,[],1),[],2),[],3); % Adjoint Solution
% end
u =  squeeze(sum(conj(sense).*ifft(ifft(ifft(datackkk.*kmask_ckkk,[],2),[],3),[],4),1)); % Adjoint Solution
...
%   for c=1:CN
%    Ku0(c,:,:,:) = fft(fft(fft(squeeze(sense(c,:,:,:)).*u_,[],1),[],2),[],3).*kmask;
%   end
  u_crrr=permute(repmat(u_,[1,1,1,CN]),[4,1,2,3]);
   Ku0 = fft(fft(fft(sense.*u_crrr,[],2),[],3),[],4).*kmask_ckkk;
   
  Ku_ = Ku0;  
  r =  Ku_ - datackkk.*permute(repmat(kmask, [1 1 1 CN]),[4 1 2 3]);% Data Fidelity gradient
  ...
  p(:,:,:,3) = p(:,:,:,3) - tau_d*(uz + xi_(:,:,:,3));
  
  % projection
  absp = sqrt(abs(p(:,:,:,1)).^2 + abs(p(:,:,:,2)).^2 + abs(p(:,:,:,3)).^2);
  ...
  g_xi2 = dym(xi_(:,:,:,2));
  g_xi3 = dzm(xi_(:,:,:,3));
  g_xi4 = (dxm(xi_(:,:,:,2)) + dym(xi_(:,:,:,1)))/2;
  g_xi5 = (dxm(xi_(:,:,:,3)) + dzm(xi_(:,:,:,1)))/2;
  g_xi6 = (dym(xi_(:,:,:,3)) + dzm(xi_(:,:,:,2)))/2;
  
  q(:,:,:,1) = q(:,:,:,1) - tau_d*g_xi1; % line
  q(:,:,:,2) = q(:,:,:,2) - tau_d*g_xi2;
  q(:,:,:,3) = q(:,:,:,3) - tau_d*g_xi3;
  q(:,:,:,4) = q(:,:,:,4) - tau_d*g_xi4; % line
  q(:,:,:,5) = q(:,:,:,5) - tau_d*g_xi5;
  q(:,:,:,6) = q(:,:,:,6) - tau_d*g_xi6;
 
  % projection
  absq = sqrt(abs(q(:,:,:,1)).^2 + abs(q(:,:,:,2)).^2 + abs(q(:,:,:,3)).^2 + 2*abs(q(:,:,:,4)).^2 + 2*abs(q(:,:,:,5)).^2 + 2*abs(q(:,:,:,6)).^2);
...
% for c=1:CN
 %  ww = ww +  conj(squeeze(sense(c,:,:,:))).*ifft(ifft(ifft(squeeze(v(c,:,:,:)).*kmask,[],1),[],2),[],3);
 % end
ww = conj(sense).*ifft(ifft(ifft(v.*kmask_ckkk,[],2),[],3),[],4);
ww = squeeze(sum(ww,1));
 ...
 %  ww = ifft(ifft(v.*kmask,[],1),[],2);
  ...
  % divergence
  divp = dxm(p(:,:,:,1)) + dym(p(:,:,:,2)) + dzm(p(:,:,:,3));
  
  u = u - tau_p*(ww + divp); %lines 8
  
  % divergence
  divq1 = dxp(q(:,:,:,1)) + dyp(q(:,:,:,4)) + dzp(q(:,:,:,5));
  divq2 = dxp(q(:,:,:,4)) + dyp(q(:,:,:,2)) + dzp(q(:,:,:,6));
  divq3 = dxp(q(:,:,:,5)) + dyp(q(:,:,:,6)) + dzp(q(:,:,:,3));
  
  xi(:,:,:,1) = xi(:,:,:,1) - tau_p*(divq1 - p(:,:,:,1));%line 11
  xi(:,:,:,2) = xi(:,:,:,2) - tau_p*(divq2 - p(:,:,:,2));%line 11
  xi(:,:,:,3) = xi(:,:,:,3) - tau_p*(divq3 - p(:,:,:,3));%line 11
...
%RelDiffSq = norm(uold-u)^2/norm(uold)^2;
  if mod(k,check_it) == 0
    ...
    dual_energy = norm(u(:))^2/2.0;
    e = [e dual_energy];
     RelDiffSq = norm(uold(:)-u(:))^2/norm(uold(:))^2;
     %imagesc((abs(Vol2Image(u))));title([num2str(k),' step']);colorbar;drawnow;
