function [raw_ctkk,kmask,header,MrProt]=CombineDatFiles_per_Coil(FilePath,CorrectVoxelShift,DoPermute)


[SCRIPT_DIR, trash1, trash2]=fileparts(mfilename('fullpath'));
CURRENT_DIR=pwd;

VoxelShift=CorrectVoxelShift;
...
if(~isempty(strfind(SoftVersionString,'N4_VD13'))| ~isempty(strfind(SoftVersionString,'E11')) | ~isempty(strfind(SoftVersionString,'E12')) )
    ...
    if isfield(header.Config,'ReadResolution')
    	...
    	MrProt.Ncol=header.Meas.BaseResolution;  
    end
    ...
    if isfield(header.Config,'VoI_PeFOV')     
    	MrProt.VOIHeight=header.Config.VoI_PeFOV;
    	MrProt.VOIWidth=header.Config.VoI_RoFOV;    
    	MrProt.VOI3D=header.Config.VoI_SliceThickness;
    elseif isfield(header.Meas,'VoiPhaseFOV')     
      	MrProt.VOIHeight=header.Meas.VoiPhaseFOV;
    	MrProt.VOIWidth=header.Meas.VoiReadoutFOV;    
    	MrProt.VOI3D=header.Meas.VoiThickness;  
    elseif isfield(header.Spice,'VoiPhaseFOV') 
        MrProt.VOIHeight=header.Spice.VoiPhaseFOV;
    	MrProt.VOIWidth=header.Spice.VoiReadoutFOV;    
    	MrProt.VOI3D=header.Spice.VoiThickness;      
    end
    
    MrProt.FlipAngle=header.Meas.FlipAngle;
    MrProt.FoVHeight=header.Config.PeFOV;
    MrProt.FoVWidth=header.Config.RoFOV;
    
    if isfield(header.MeasYaps.sSliceArray.asSlice{1},'dThickness')
        MrProt.FoV3D=header.MeasYaps.sSliceArray.asSlice{1}.dThickness;
    elseif isfield(header.Config,'VoI_SliceThickness')
    ...
    if isfield(header.MeasYaps,'alTE')
    	MrProt.TE=header.MeasYaps.alTE{1}*1E-6 ; % in s
    end
...
elseif(~isempty(strfind(SoftVersionString,'B17')))
    ...
    comb_twix.raw_tckk=ResizeDataToRightSize(comb_twix.raw_tckk,MrProt.Nlines,MrProt.Ncol,MrProt.Nslc);
...
if NDim==2
  ...
  %  kmask=flip(permute( kmask,[2,1]),1);
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %1Voxel shift measured empirically   
    comb_twix.raw_tcrr=circshift(circshift(comb_twix.raw_tcrr,1,3),-1,4);
   else
    comb_twix.raw_tcrr=circshift(circshift(comb_twix.raw_tcrr,-1,3),-1,4); 
   end
...
[Kr,Kc,Ks] = meshgrid(0:(size(comb_twix.raw_tckk,4)-1),0:(size(comb_twix.raw_tckk,3)-1),0:(size(comb_twix.raw_tckk,5)-1));
	...
	Ks=2*pi*Ks/size(comb_twix.raw_tckk,5)*double(~mod(size(comb_twix.raw_tckk,5),2));

	comb_twix.raw_tckk=comb_twix.raw_tckk.* reshape(exp(1i*VoxelShift*Kr).*exp(1i*VoxelShift*Kc).*exp(1i*VoxelShift*Ks),[1 1 size(Kr)]);
...
if isfield(comb_twix,'PtsBeforeEcho')
	MrProt.PtsBeforeEcho=comb_twix.PtsBeforeEcho;
end
